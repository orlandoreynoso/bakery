...
SELECT order_id,item_id,attributes,sku,quantity,price,tax_rate FROM co_mod_bakery_order WHERE order_id = '26'
Original
echo $consulta200 =  "SELECT * FROM " .TABLE_PREFIX."mod_bakery_order WHERE order_id = '$order_id'";

*/

$consulta200 =  "SELECT order_id,item_id,attributes,quantity  FROM " .TABLE_PREFIX."mod_bakery_order WHERE order_id = '$order_id'";

$sql_result1 = $database->query($consulta200);
...
while ($row1 = $sql_result1->fetchRow()) {
	foreach ($row1 as $field => $value) {
		if ($field != "order_id") {
			...
			/* original
				SELECT title, shipping, link FROM co_mod_bakery_items WHERE item_id = '214'
			*/
			if ($field == "item_id") {
				
				$consulta201 = "SELECT title, link FROM ".TABLE_PREFIX."mod_bakery_items WHERE item_id = '".$row1['item_id']."'";				
				$sql_result2 = $database->query($consulta201);
				...
				$main_thumb = '';
				$consulta202 = "SELECT `filename` FROM ".TABLE_PREFIX."mod_bakery_images WHERE `item_id` = '{$row1['item_id']}' AND `active` = '1' ORDER BY `position` ASC LIMIT 1";
				$main_thumb = $database->get_one($consulta202);
